...
class LogPreProcess(DataValidation):
    ...
    def __init__(self, file_path: str):
        ...
        self.columns = sorted( ['userId', 'firstName', 'lastName', 'gender', 'level',
                                ...
                                'artist', 'length', 'song'])  
        self.dtypes = log_dtype_dict = {'userId': int,
                                        'firstName': str, 
                                        'lastName': str,
                                        'gender': str,
                                        'level': str,
    ...
    def create_songplays_help_df(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Returns a dataframe which can be combined with artists and songs to create the final songplays table in Postgres.
        """
...
class SongPlaysPreProcess:
    ...
    def merge_dataframes(self, artists_df, songs_df):
        """
        Executes the merge methods, and simple transformations, to prepare the songplays table for insertion in Postgres.
        """
        ...
        songs_artists = songs_artists.merge(songs_df, how='left', left_on=['song', 'length'], right_on=['title', 'duration'])
        songs_artists['start_time'] = pd.to_datetime(songs_artists['ts'], unit='ms')
        
        songplays_columns = ['start_time', 'userId', 'level', 'song_id', 'artist_id_x', 'sessionId', 'location_x', 'userAgent']
        
        return songs_artists[songplays_columns]
    
    def create_postgres_insert_lists(self, df: pd.DataFrame) -> list:
        """
        Returns a list of tuples with standard python dtypes, these dtypes are necessary for Postgres.
        """ 
        songplays = [tuple(row) for row in df.itertuples(index=False)]
